...
public class PublicKeyFetcher {
    ...
    /**
     * 初始化加密相關的金鑰
     * @param serverUrl 伺服器的 URL，例如 "https://iam.digiwincloud.com.cn"
     */
    public void initialize(String serverUrl) throws Exception {
        // 取得服務器公鑰
        String serverPublicKeyStr = fetchServerPublicKey(appToken, serverUrl);
     ...
     * 執行登入流程
     */
    public String login(String userId, String password, String serverUrl) throws Exception {
        ...
        // 獲取 AES 金鑰
        String encryptedAesKeyFromServer = fetchAesKeyFromServer(appToken, encryptedAESKey, serverUrl);
        // 需要 serverUrl 參數，需保存 serverUrl
        // 解密 AES 金鑰
        ...
        // 執行登入
        this.token = loginAndGetToken(appToken, userId, encryptedPassword, encryptedAESKey, serverUrl);
        return this.token;
    }
     ...
     * 向 AI 發送問題
     */
    public String askQuestion(String question,String aiServerUrl) throws Exception {
        if (token == null) {
            throw new IllegalStateException("Please login first");
        }
        if (aiServerUrl == null) {
            throw new IllegalStateException("Please set AI server URL first");
        }
        return askAI(token, question, aiServerUrl);
    }
    ...
    public static String fetchServerPublicKey(String appToken, String serverUrl) throws Exception {
        URL url = new URL(serverUrl + "/api/iam/v2/identity/publickey");
    ...
    //         // 使用封裝的登入與問答流程
    //         String token = fetcher.login(userId, password, serverUrl);
    //         System.out.println("登入取得的 Token: " + token);

    //         String question = "幫我介紹一下這家公司";
    //         String aiReply = fetcher.askQuestion(question,aiServerUrl);
    ...
    public static String fetchAesKeyFromServer(String appToken, String encryptedClientPublicKey, String serverUrl) throws Exception {
        ...
        URL aesUrl = new URL(serverUrl + "/api/iam/v2/identity/aeskey");
    ...
    // 發送登入請求取得 token
    public static String loginAndGetToken(String appToken, String userId, String encryptedPassword, 
            String encryptedClientPublicKey, String serverUrl) throws Exception {
        ...
        URL loginUrl = new URL(serverUrl + "/api/iam/v2/identity/login");
    ...
    // 使用 token 向 AI 發問
    public static String askAI(String token, String question, String aiServerUrl) throws Exception {
        JSONObject aiRequest = new JSONObject();
        aiRequest.put("question", question);

        URL indepthUrl = new URL(aiServerUrl);
